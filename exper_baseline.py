...
parser.add_argument('--sigma',default='0.1,0.3,0.5,0.7,1')
...
models = {
            ...
            'KPI':KPI(batch_size=512, lr=5e-3, n_epochs=args.epochs, n_pairs=2, noise=1e-4, labda=1.0, normalize=1, initializer=KNNImputation(k=5, weights="distance"), replace=False,sigma=args.sigma),
        ...
        'kip':KIPImputation(batch_size=512, lr=5e-4, n_epochs=50, n_pairs=2, noise=1e-4, labda=1.0, normalize=1, initializer=KNNImputation(k=5, weights="distance"), replace=False),
        'mean': MeanPlugin(),
        'median': MedianPlugin(),
        ...
        'mice': MicePlugin(n_imputations=5, max_iter=100, ),
        ...
        'knn': KNNImputation(k=1, weights="distance"),  # 1-3
        'knn-l': KNNImputation(k=3, weights="distance"),  # 1-3
    ...
    for p_miss in P_MISS:
        for feature_miss in feature_drop:
            for model_name in args.model.split(','):
                ...
                x, x_miss, mask = imputation_scenarios[scenario][p_miss][feature_miss]
                ...
                print(result)
                if model_name in ['multikip','multilaplaciankip','multilinearkip','multipolykip']:
                    with open(f'./results/{args.dataset_name}/{model_name}_{scenario}_{p_miss}_{feature_miss}.yaml','w') as file:
                ...
                result_dict = {"model_name": model_name, "missing": scenario, "p_miss": p_miss,
                            "seed": args.seed, "rmse": rmse, "mae": mae, "wass": wass}
