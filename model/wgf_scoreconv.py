...
class NeuralGradFlowImputer(object):
    ...
    def fit_transform(self, X, verbose=True, report_interval=10, X_true=None, OTLIM=5000):
        ...
        for i in range(self.niter):
            ...
            total_loss_list = self.train_score_net(train_dataloader, i, score_trainer)
            ...
            score_loss_dataframe = pd.DataFrame({"iter": list(map(lambda x: x, range(len(total_loss_list)))), "score_loss": total_loss_list})
            score_loss_dataframe["iter_idx"] = i + 1
            score_dataframe_list.append(score_loss_dataframe)

        # X_filled = X.detach().clone()
        # X_filled[mask.bool()] = imps

        return pd.concat(score_dataframe_list, axis=0)
